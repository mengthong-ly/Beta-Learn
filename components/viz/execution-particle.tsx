 ...
 */
export function ExecutionParticle({ d, seconds }: { d: string; seconds: number }) {
  ...
  return (
    <circle r={4} opacity={0} style={{ fill: "var(--iso-accent)" }} aria-hidden>
      <animateMotion ref={move} path={d} dur={`${seconds}s`} begin="indefinite" fill="freeze" calcMode="spline" keyTimes="0;1" keySplines="0.4 0 0.2 1" />
      <animate ref={fade} attributeName="opacity" values="0;1;1;0" keyTimes="0;0.1;0.85;1" dur={`${seconds}s`} begin="indefinite" fill="freeze" />
    </circle>
  )
}
