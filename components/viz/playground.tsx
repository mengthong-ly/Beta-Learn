    ...
    <main className="flex min-h-svh flex-col lg:h-svh">
      <header className="flex items-center gap-4 border-b px-4 py-3">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-muted-foreground">Watch what the computer does, one operation per step.</p>
        </div>
      </header>

      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <nav aria-label="Demos" className="shrink-0 border-b lg:w-52 lg:overflow-y-auto lg:border-r lg:border-b-0">
          <ol className="flex gap-1 overflow-x-auto p-2 lg:flex-col">
            {DEMOS.map((d, i) => (
              <li key={d.id} className="shrink-0">
                <button
                  onClick={() => router.replace(`${pathname}?demo=${d.id}`, { scroll: false })}
                  aria-current={d.id === demo.id ? "page" : undefined}
                  className={cn(
                    "flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-sm whitespace-nowrap text-muted-foreground transition-colors hover:bg-accent hover:text-foreground",
                    d.id === demo.id && "bg-accent font-medium text-foreground"
                  )}
                >
                  <span className="w-4 text-right font-mono text-[11px] tabular-nums opacity-70">{i + 1}</span>
                  {d.title}
                </button>
              </li>
            ))}
...
function DemoView({ demo }: { demo: Demo }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
      <section className="relative order-1 h-[56svh] min-h-80 shrink-0 lg:order-2 lg:h-auto lg:flex-1" aria-label="Visualization">
        {/* absolute: React Flow sizes itself to 100% of its parent, which a flex item's height isn't */}
        <div className="absolute inset-0">
          <VizCanvas key={demo.id} layout={layout} index={player.index} forward={player.forward} speed={player.speed} />
        </div>
      </section>
