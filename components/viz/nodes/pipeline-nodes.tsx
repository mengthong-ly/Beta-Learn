...
import { CARD, CONSOLE, outputCard, stage, STAGE, type OutputData, type StageData } from "@/lib/viz/layout"
...
export function StageNode({ data }: NodeProps<Node<StageData, "stage">>) {
  ...
  return (
    <NodeCanvas canvas={g} label={`${data.title}: ${data.payload ? data.payload.join("; ") : "waiting"}`}>
      <motion.g animate={{ opacity: lit ? 1 : 0.55 }} transition={{ duration: dur(0.3) }}>
        <IsoBox box={STAGE} tone={data.state === "active" ? "accent" : "neutral"} />
      ...
      <g transform={`translate(${card.x},${card.y})`}>
        <rect
          ...
          style={{
            fill: "var(--card)",
            stroke: data.state === "active" ? "var(--iso-accent)" : "var(--border)",
            strokeWidth: 1,
            transition: "stroke 240ms ease",
          }}
        />
        <text x={10} y={17} style={{ fontSize: 11, fontWeight: 600, fill: lit ? "var(--foreground)" : "var(--muted-foreground)" }}>
          {data.title}
        </text>
        <AnimatePresence>
          {data.payload?.map((line, i) => (
            <motion.text
              ...
              className="font-mono"
              style={{ fontSize: 11, fill: "var(--foreground)", whiteSpace: "pre" }}
              initial={{ opacity: 0, x: 4 }}
              animate={{ opacity: 1, x: 10 }}
              exit={{ opacity: 0 }}
              transition={{ duration: dur(0.25), delay: dur(0.035 * i), ease: EASE }}
            >
              {line}
            </motion.text>
          ))}
...
export function OutputNode({ data }: NodeProps<Node<OutputData, "output">>) {
  ...
  return (
    <div
      ...
      role="img"
      className={cn("relative overflow-hidden rounded-lg border bg-card shadow-xs transition-colors", data.active && "border-(--iso-accent)")}
      style={{ width: g.width, height: g.height }}
    >
      <div className="border-b px-3 text-[11px] font-semibold tracking-wide text-muted-foreground uppercase" style={{ height: CONSOLE.head, lineHeight: `${CONSOLE.head}px` }}>
        Output
      </div>
