...
export function GateNode({ data }: NodeProps<Node<GateData, "gate">>) {
  return (
    <NodeCanvas
      canvas={GATE_CANVAS}
      label={`if ${data.expr}: ${data.result === null ? "not evaluated yet" : `evaluated to ${data.result ? "True" : "False"}`}`}
    >
...
export function BranchNode({ data }: NodeProps<Node<BranchData, "branch">>) {
  ...
  return (
    <NodeCanvas
      canvas={BRANCH_CANVAS}
      label={`${data.label} branch: ${data.code}${data.state === "idle" ? "" : data.state === "taken" ? ", runs" : ", skipped"}`}
    >
      <motion.g
        animate={{ opacity: data.state === "skipped" ? 0.38 : 1 }}
        transition={{ duration: dur(0.3) }}
      >
        <IsoBox
          box={BRANCH}
          tone={data.state === "taken" ? "accent" : "neutral"}
        />
