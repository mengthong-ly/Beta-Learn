 ...
 */
export function IsoGate({ box, expr, result }: { box: Box; expr: string; result: boolean | null }) {
  const dur = useDur()
  ...
  return (
    <g>
      ...
      <Tag at={{ x: t.x - 8, y: t.y + 12 }} anchor="end" tone={result === true ? "accent" : "muted"}>
        True
      </Tag>
      <Tag at={{ x: f.x + 8, y: f.y + 12 }} anchor="start" tone={result === false ? "accent" : "muted"}>
        False
      </Tag>
      {result !== null && (
        <motion.g initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: dur(0.3), ease: EASE }}>
          <Tag at={{ x: top.x + 10, y: top.y - 6 }} anchor="start" tone="accent" mono>
            → {result ? "True" : "False"}
          </Tag>
        </motion.g>
      )}
