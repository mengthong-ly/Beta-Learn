...
export const project = ({ x, y, z = 0 }: P3): P2 => ({ x: (x - y) * C, y: (x + y) * S - z })
...
export function boxFaces({ w, d, h }: Box, at: P3 = { x: 0, y: 0 }) {
  const z0 = at.z ?? 0
  const p = (x: number, y: number, z: number) => project({ x: at.x + x, y: at.y + y, z: z0 + z })
  return {
...
export const onFloor = (p: P2) => `matrix(${r(C)},${S},${r(-C)},${S},${r(p.x)},${r(p.y)})`
 ...
 */
export function isoPath(a: P2, b: P2, kind: PathKind): { d: string; angle: number } {
  const dx = b.x - a.x
  const dy = b.y - a.y
  if (kind === "straight" || (dx === 0 && dy === 0))
    return { d: `M${r(a.x)},${r(a.y)} L${r(b.x)},${r(b.y)}`, angle: Math.atan2(dy, dx) }
  if (kind === "hcurve") {
  ...
  const last = Math.abs(t) > 0.5 ? { x: -C * Math.sign(t), y: S * Math.sign(t) } : { x: C * Math.sign(s), y: S * Math.sign(s) }
  return {
