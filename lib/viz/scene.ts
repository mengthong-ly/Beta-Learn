...
export function reduceScene(s: Scene, ev: VizEvent, before: Program, after: Program): Scene {
  let next = s.next
  const fresh = () => `e${next++}`
  ...
  switch (ev.type) {
    ...
    case "array.create": {
      ...
      return { ...s, ids: withIds(list, ids), next, focus: { kind: "list", list } }
    }
    case "array.insert": {
      const list = listId(before, ev.name)
      const ids = [...s.ids[list]]
      ids.splice(ev.index, 0, fresh())
      return { ...s, ids: withIds(list, ids), next, focus: { kind: "element", list, index: ev.index } }
    }
    case "array.remove": {
      const list = listId(before, ev.name)
      const ids = s.ids[list].filter((_, i) => i !== ev.index)
      return { ...s, ids: withIds(list, ids), focus: ev.into ? { kind: "var", name: ev.into } : { kind: "list", list } }
    }
    case "array.set":
    case "array.access":
      return { ...s, focus: { kind: "element", list: listId(before, ev.name), index: ev.index } }
    case "loop.iter":
      return { ...s, focus: { kind: "element", list: listId(before, ev.array), index: ev.index } }
    case "loop.end":
      return { ...s, focus: { kind: "list", list: listId(before, ev.array) } }
    case "cond.eval":
      return { ...s, focus: { kind: "branch", id: ev.id, result: ev.result } }
    case "call":
      return { ...s, frames: [...s.frames, fresh()], next, focus: { kind: "fn", fn: ev.fn, depth: after.frames.length } }
    case "return":
      return { ...s, frames: s.frames.slice(0, -1), focus: { kind: "fn", fn: ev.fn, depth: after.frames.length } }
    case "pipeline.stage":
