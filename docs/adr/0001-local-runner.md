...
**Status:** Accepted, 2026-09-22. Partly superseded by [ADR-0002](0002-browser-runtimes.md) (2026-09-23): PHP, Laravel, TypeScript and Claude Code now run in the browser, the runner covers only C++, Dart and Flutter, and the website never calls it.
...
3. The custom header `x-thonglearn-run: 1` is present. That forces a CORS preflight, which is never approved, so no other website can post to the runner. (An opt-in `RUNNER_ORIGINS` bridge for the hosted site existed briefly; ADR-0002 removed it.)
...
5. Each run is inside an OS sandbox (`lib/sandbox.ts`, `@anthropic-ai/sandbox-runtime`: `sandbox-exec` on macOS, bubblewrap on Linux). A run's output goes back to the page, so anything it can read, it can leak. The sandbox therefore enforces:
   ...
   If the sandbox can't start (Windows, or Linux without bubblewrap), nothing runs. `THONGLEARN_UNSANDBOXED=1` turns it off for local debugging.

## Consequences

- Courses that run locally need ThongLearn running on the learner's machine (`npm run setup:runtimes` once, then `npm run dev`). On the website they are write-only (ADR-0002).
