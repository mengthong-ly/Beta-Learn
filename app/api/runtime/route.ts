...
export async function GET(request: Request) {
  return refuse(request) ?? Response.json(await snapshot())
}
...
export async function POST(request: Request) {
  ...
  if (!CACHE_IDS.includes(id)) return Response.json({ error: "bad request" }, { status: 400 })
  try {
    await clean(id)
  } catch (e) {
    return Response.json({ error: e instanceof Error ? e.message : String(e), ...(await snapshot()) }, { status: 500 })
  }
